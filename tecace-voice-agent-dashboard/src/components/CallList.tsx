...
import { deleteInboundCall } from "../api/backend";
import type { InboundCall } from "../api/types";
...
//
// Shared by the single-business page and each business's panel, so a call reads the same wherever it
// is shown.
...
export function CallList({
  ...
  showWho = false,
  onDeleted,
  emptyText = "No calls yet. When someone rings and the assistant answers, the conversation appears here.",
}: {
  calls: InboundCall[];
  showWho?: boolean;
  onDeleted: () => void;
  ...
  return (
    <ul className="call-list">
      {calls.map((call) => (
        <CallRow key={call.id} call={call} showWho={showWho} onDeleted={onDeleted} />
      ))}
    </ul>
  );
}
