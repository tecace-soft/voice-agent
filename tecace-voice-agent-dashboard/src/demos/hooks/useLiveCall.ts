...
export function useLiveCall(
  ...
  const report = useCallback(
    (status: "completed" | "failed" | "abandoned", endReason?: string) => {
      ...
      // The promo had a `beacon` path here: `navigator.sendBeacon` for the
      // unload case, this fetch otherwise. A beacon cannot carry a header, so
      // it cannot carry the bearer token this backend's admin guard wants, and
